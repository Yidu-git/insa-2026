...
# for SSTI from jinja2.sandbox import SandboxedEnvironment 
from flask import ( Flask, request, render_template, render_template_string, redirect, url_for, session, make_response, jsonify, g, send_from_directory, ) 
from werkzeug.utils import secure_filename 

app = Flask(__name__) 
app.secret_key = "supersecretkey_vulncorp_2024" 
...
ALLOWED_EXTENSIONS_WEAK = {"png", "jpg", "jpeg", "gif", "pdf", "txt", "php", "phtml"} 
...
def get_db(): 
    if "db" not in g: 
        ...
        return g.db @app.teardown_appcontext 
...
def init_db(): 
    ...
    db.executescript(f""" CREATE TABLE IF NOT EXISTS users ( id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT UNIQUE NOT NULL, password TEXT NOT NULL, role TEXT DEFAULT 'user', email TEXT, notes TEXT DEFAULT '', approved INTEGER DEFAULT 0 ); CREATE TABLE IF NOT EXISTS documents ( id INTEGER PRIMARY KEY AUTOINCREMENT, owner_id INTEGER, title TEXT, content TEXT, is_private INTEGER DEFAULT 1 ); CREATE TABLE IF NOT EXISTS feedback ( id INTEGER PRIMARY KEY AUTOINCREMENT, author TEXT, message TEXT, reviewed INTEGER DEFAULT 0 ); CREATE TABLE IF NOT EXISTS profiles ( id INTEGER PRIMARY KEY, bio TEXT, department TEXT, template_field TEXT ); INSERT OR IGNORE INTO users (username, password, role, email, notes, approved) VALUES ('admin', '5f4dcc3b5aa765d61d8327deb882cf99', 'admin', 'admin@vulncorp.internal', 'Admin account - do not share', 1), ('alice', 'ef92b778bafe771e89245b89ecbc08a6', 'user', 'alice@vulncorp.com', '{FLAG_IDOR}', 1), ('bob', '1a79a4d60de6718e8e5b326e338ae533', 'user', 'bob@vulncorp.com', 'Nothing special here', 1), ('charlie', 'a87ff679a2f3e71d9181a67b7542122c', 'user', 'charlie@vulncorp.com', 'I love CTFs', 1); INSERT OR IGNORE INTO documents (owner_id, title, content, is_private) VALUES (1, 'Admin Secret', 'Top secret data. {FLAG_BAC}', 1), (2, 'Alice Notes', 'My personal notes.', 1), (3, 'Bobs Public', 'Nothing to see.', 0), (1, 'Server Creds', 'ssh root@10.10.10.1 pass=r00t_v4ult', 1); INSERT OR IGNORE INTO profiles (id, bio, department, template_field) VALUES (1, 'Site administrator.', 'IT', 'Welcome Admin'), (2, 'Developer.', 'Engineering', 'Welcome Alice'); """)
    db.commit() 
    ...
    if "approved" not in user_cols: 
        ...
        db.execute( "UPDATE users SET approved=1 WHERE username IN ('admin','alice','bob','charlie')" ) 
        db.commit() 
...
# ── BAC: /document/<id> ─────────────────────────────────────────────────────── 
@app.route("/document/<string:doc_key>") 
@login_required 
def view_document(doc_key): 
    db = get_db() 
    # VULN: BAC — fetches any document regardless of ownership 
    doc = db.execute("SELECT * FROM documents WHERE doc_key=?", (doc_key,)).fetchone() 
    if not doc: 
        return render_template("error.html", msg="Document not found.", code=404), 404 
...
def feedback(): 
    ...
    if request.method == "POST": 
        ...
        # VULN: stored XSS — message not sanitised 
        db.execute( "INSERT INTO feedback (author, message) VALUES (?, ?)", (author, message) ) 
        db.commit() 
        msg = "Feedback submitted! An admin will review it shortly." 
        entries = db.execute("SELECT * FROM feedback ORDER BY id DESC LIMIT 20").fetchall() 
        return render_template("feedback.html", entries=entries, msg=msg) 
    
# Admin feedback review (bot visits this — XSS target) 
